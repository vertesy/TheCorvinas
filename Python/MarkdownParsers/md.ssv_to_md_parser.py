#!/usr/bin/python
'''
### md.ssv_to_md_parser.py
- read a space delimeted file [typical bash output] and print the output in [github] .md format
...
with open(tsv, 'rU') as textfile:									# U for Universal line ending
	for row in (list(csv.reader(textfile, delimiter=' ', skipinitialspace=True))):
		i +=1
		if i ==1:
			nr_cols = len(row)
