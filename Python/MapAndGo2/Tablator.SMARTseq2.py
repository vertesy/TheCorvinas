...
# - Test it by running in bash: `python path/to/Tablator.SMARTseq.py`
# - Run it with your data: python `python path/to/Tablator.SMARTseq.py path/to/InputSam 1` # Put 1 (or 2 if you used CELseq2 primers)
# - Original author: Anna Alemany, van Oudenaarden group, 25-10-2016
# - Modified  by Abel Vertesy, van Oudenaarden group, 25-10-2016
# ## Loaction: /home/hub_oudenaarden/avertesy/bin/Mapping/Tablator.SMARTseq.py

"""
Changelog is in the file (two):
- @line154: sorted(readscnt) instead sorted(umicnt)
- Only one table is written
- isReverseStrand() disabled

"""


print "Tablator.SMARTseq.py Started"
import sys, os
import numpy as np
...
from CELSeq2_384BC import bc2sample as bc2sample 	 # cell barcodes for CELseq 2
bclen = 8
umilen = 6
...
with open(InputSamFile) as f:
	for line in f:
		...
		if read.isMapped() and read.isUniquelyMapped():
			gene = read.rname
...
print "CountingTablator.SMARTseq.py Finished"
